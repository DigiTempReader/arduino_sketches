...
volatile boolean continue_further                       =false;

//char file_list[10][13]={'\0'};
...
void setup () 
  ...
  // Print a message to the LCD.
     lcd.print(F("HEX Injector!"));
     
  pinMode(upload_key,INPUT);
...
void setup_gsm()
{
 // Serial.begin(9600);
 // while (!Serial);
  Serial.println(F("Entering GSM section !"));
...
if(file_size==no_of_bytes_recieved)
  {
    ...
    Serial.println(F("\nCorrect file recieved"));
  }

}


void show_total_files()
{
    lcd.setCursor(14,1);
    lcd.print((char)total_files,DEC);
}
...
if(digitalRead(upload_key)==HIGH)
  {
   Serial.println(F("Upload key pushed : Burning my hex file"));
   create_hex_file();
   setup_gsm();
   if(continue_further==true)
     {
     continue_further=false;
     Serial.println(F("continuing..."))  ;
     if(myFile2.fileSize()==no_of_bytes_recieved)
     {
       Serial.println(F("correct size, writing"));
       strcpy(name,"RECVD.HEX");
       write_my_hex();
       stopProgramming();
     }  
   else
   {
     Serial.print(F("size mismatch error, given size : \t"));
     Serial.println(myFile2.fileSize());
     
   }
  }
   else
     {
       Serial.println(F("continue_further False :  HALTING"));
       read_hex_file();
     }
  }  
} 
...
void write_my_hex()
{
  ...
  if (foundSig == -1)
    ...
    valid_signature=true;
      ...
      Serial.print(F("SD CARD value is\t"));
      Serial.println(haveSDcard);
      Serial.print(F("Signature validity is\t"));
      Serial.println(valid_signature);
 ...
 while(my_file_complete==false)
  {
    if(i==100)
     { 
       myFile2.write(buf,100);
       i=0;
       //strcpy(buf,"\0");
     }
    while(counter==0)
    {
     counter=mySerial.available();
    }
...
if(my_file_complete==true)
  ...
  {
    Serial.println(F("magic String NOT recieved"));
    Serial.print(F("no of bytes recieved \t"));
    Serial.print(no_of_bytes_recieved);
...
void read_hex_file()
{
Serial.println(F("Reading HEX file so recieved"));
if(sd.exists("RECVD.HEX"))
  Serial.println(F("file exists, now reading"));
else
  Serial.println(F("No scch file exists"));
