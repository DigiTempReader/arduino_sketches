...
//    unsigned char file_no=0;
  ...
  while (file.openNext(sd.vwd(), O_READ)) {
    ...
    if (len > 4 && strcmp (&name [len - 4], ".HEX") == 0)
      {
      Serial.print (name);
      strcpy(file_list[total_files],name);
      //lcd.setCursor(1,1);
      //lcd.print(file_list[file_no]);
      //delay(1000);
      total_files++;
...
void loop () 
{
 ...
 if(digitalRead(Up_key)==HIGH)
  {
    Serial.print(F("current file no. is\t"));
    Serial.println(current_file,DEC);
   if(current_file>=0 && current_file<total_files)
   {
     strcpy(hex_file_name, file_list[current_file]);
     lcd.setCursor(1,1);
     lcd.print("            ");
     lcd.setCursor(1,1);
     lcd.print(file_list[current_file]);
     current_file++;
...
if(digitalRead(Down_key)==HIGH)
 ...
 if(digitalRead(Enter_key)==HIGH)
  {
   //strcpy(hex_file_name,"BLINK2.HEX");  
   Serial.println(F("Pin HIGH : Burning my hex file"));
   write_my_hex();
   stopProgramming();
   //delay(5000);
  }
