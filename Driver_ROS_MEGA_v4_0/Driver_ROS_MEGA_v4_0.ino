...
volatile uint8_t rc_flag=HIGH;//global flag for RC ON/OFF status
...
void motor_cb( const std_msgs::UInt16& cmd_msg)
{
  ...
  if(cmd_msg.data == 2 && rc_flag==0)
   {
    move_forward(70);
    delay(50);
    move_stop();
   }
  else if(cmd_msg.data == 8 && rc_flag==0 )
   {
    move_backward(70);
    delay(50);
    move_stop();
   }
  else if(cmd_msg.data == 4 && rc_flag==0)
   {
    move_turn(70);
    delay(50);
    move_stop();
   }
  else if(cmd_msg.data == 6 && rc_flag==0)
   {
    move_turn(-70);
    delay(50);
    move_stop();
...
ros::Subscriber<std_msgs::UInt16> sub("my_bot", motor_cb);
